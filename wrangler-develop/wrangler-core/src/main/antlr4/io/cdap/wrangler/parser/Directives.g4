...
directive
 : command
  (   codeblock
    ...
    | numberRanges
    | properties
    | byteSize            // Added ByteSize to the directive rule
    | timeDuration        // Added TimeDuration to the directive rule
  )*?
  ;
...
/*
 * Lexer Rules used for tokenizing the recipe.
 */

Bytesize
    : [0-9]+ ( 'KB' | 'MB' | 'GB' | 'TB' )  // Matches ByteSize patterns like 5GB, 10MB
    ;

TimeDuration
    : [0-9]+ ( 'ms' | 's' | 'm' | 'h' | 'd' )  // Matches Time Duration patterns like 30m, 2h
    ;
